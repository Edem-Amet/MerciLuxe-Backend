    ...
    try {
        await Promise.all(
            req.files.map(file => new Promise((resolve, reject) => {
                ...
                const folder = isVideo ? 'bead_videos' : 'bead_images'; // ✅ bead-specific folders
        ...
        console.error('Bead upload failed:', error);
        res.status(500).json({ error: 'Bead media upload failed' });
    }
};
